...
func findNearestValidSpawnTile(
    startX: Int,
    startY: Int,
    occupied: Set<String>,
    map: [[Int]]
    ...
    func valid(_ x: Int, _ y: Int) -> Bool {
        ...
        guard isWalkable(map[y][x]) else { return false }
        return !occupied.contains(tileKey(x: x, y: y))
    }

    if valid(startX, startY) { return (startX, startY) }
...
func auditFixture(_ fixture: MapFixture, situation: MapSituation) -> AuditResult {
    ...
    var playerSpawn = fixture.playerSpawn
    var enemySpawns = fixture.enemies
    var flags: [String] = []
    ...
    // Mirror runtime spawn-safety relocation before validation.
    var occupied = Set<String>()
    if let safe = findNearestValidSpawnTile(startX: playerSpawn.x, startY: playerSpawn.y, occupied: occupied, map: shapedMap) {
        if safe.x != playerSpawn.x || safe.y != playerSpawn.y {
            notes.append("Player spawn relocated for safety: (\(playerSpawn.x),\(playerSpawn.y))->(\(safe.x),\(safe.y)).")
        }
        playerSpawn = SpawnPoint(x: safe.x, y: safe.y)
        occupied.insert(tileKey(x: safe.x, y: safe.y))
    } else {
        flags.append("SPAWN_BLOCKED")
        notes.append("Player spawn invalid after shaping and could not be relocated.")
    }

    for i in enemySpawns.indices {
        let enemy = enemySpawns[i]
        if let safe = findNearestValidSpawnTile(startX: enemy.x, startY: enemy.y, occupied: occupied, map: shapedMap) {
            if safe.x != enemy.x || safe.y != enemy.y {
                notes.append("Enemy spawn relocated for safety: (\(enemy.x),\(enemy.y))->(\(safe.x),\(safe.y)).")
            }
            enemySpawns[i] = EnemySpawn(type: enemy.type, x: safe.x, y: safe.y, delay: enemy.delay)
            occupied.insert(tileKey(x: safe.x, y: safe.y))
        } else {
            flags.append("SPAWN_BLOCKED")
            notes.append("Enemy spawn invalid after shaping and could not be relocated: (\(enemy.x),\(enemy.y)).")
        }
    }

    // Duplicate check after relocation.
    occupied.removeAll()
    occupied.insert(tileKey(x: playerSpawn.x, y: playerSpawn.y))
    ...
    let reachable = bfsReachable(map: shapedMap, from: playerSpawn, to: extractionPoint)
    if !reachable {
        flags.append("NO_EXTRACTION_PATH")
    }
