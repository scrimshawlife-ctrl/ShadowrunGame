...
final class GameState: ObservableObject {
    ...
    private func escalatedIncomingDamage(_ baseDamage: Int) -> Int {
        ...
        var adjustedDamage = dynamicBonus > 0 ? (baseDamage + dynamicBonus) : baseDamage
        if playerRole == .street {
            addLog("STREET — bracing against escalation")
            let reducedDamage = max(0, adjustedDamage - 1)
            addLog("STREET — reduced incoming damage")
            adjustedDamage = reducedDamage
        }
        return max(0, adjustedDamage + situationIncomingDamageModifier())
    }

    private func situationIncomingDamageModifier() -> Int {
    ...
    private func missionStartSituationEffectLogLine() -> String {
        switch currentMapSituation {
        case .corridor:
            return "Corridor advantage: cover reduces incoming damage."
        case .openZone:
            return "Open terrain: exposure increases incoming damage."
        case .chokepoint:
            return "Chokepoint pressure: routes are constrained."
        }
    }
    ...
    func situationTacticalHint() -> String {
        switch currentMapSituation {
        case .corridor:
            return "Movement is constrained. Use cover and control the lane."
        case .openZone:
            return "Exposure is high. Positioning and threat awareness are critical."
        case .chokepoint:
            return "Routes are narrow. Movement decisions will determine success."
        }
    }

    var situationIntelLine: String {
        "\(mapSituationLabel) — \(situationShortHint)"
    }

    var situationEffectDescription: String {
        switch currentMapSituation {
        case .corridor:
            return "Incoming damage reduced by 1 (min 0)."
        case .openZone:
            return "Incoming damage increased by 1."
        case .chokepoint:
            return "Extraction pressure emphasized. No damage change."
        }
    }

    private var situationShortHint: String {
        let shortHint: String
        switch currentMapSituation {
        case .corridor:
            shortHint = "cover and lane control"
        case .openZone:
            shortHint = "high exposure, multiple angles"
        case .chokepoint:
            shortHint = "narrow routes, extraction funnel"
        }
        return shortHint
    }
    ...
    @Published var preparedMissionId: String?
    @Published var lastSetupTraceToken: String?
    ...
    private func isWalkableSpawnTile(_ tile: Int) -> Bool {
        tile == TileType.floor.rawValue ||
        tile == TileType.cover.rawValue ||
        tile == TileType.door.rawValue ||
        tile == TileType.extraction.rawValue
    }

    private func findNearestValidSpawnTile(
        startX: Int,
        startY: Int,
        occupied: Set<String>,
        map: [[Int]]
        ...
        func valid(_ x: Int, _ y: Int) -> Bool {
            guard y >= 0, y < height, x >= 0, x < width else { return false }
            guard isWalkableSpawnTile(map[y][x]) else { return false }
            return !occupied.contains(tileKey(x: x, y: y))
        }
    ...
    private func enforceSpawnSafety() {
        var occupied = Set<String>()

        for (index, char) in playerTeam.enumerated() where char.isAlive {
            let oldX = char.positionX
            let oldY = char.positionY
            if let safe = findNearestValidSpawnTile(startX: oldX, startY: oldY, occupied: occupied, map: currentMissionTiles) {
                if safe.x != oldX || safe.y != oldY {
                    char.positionX = safe.x
                    char.positionY = safe.y
                    addLog("Spawn relocated for safety: (\(oldX), \(oldY)) -> (\(safe.x), \(safe.y))")
                }
                occupied.insert(tileKey(x: char.positionX, y: char.positionY))
            } else {
                addLog("Spawn safety error: no valid player tile for \(char.name) [\(index)].")
            }
        }

        for (index, enemy) in enemies.enumerated() where enemy.isAlive {
            let oldX = enemy.positionX
            let oldY = enemy.positionY
            if let safe = findNearestValidSpawnTile(startX: oldX, startY: oldY, occupied: occupied, map: currentMissionTiles) {
                if safe.x != oldX || safe.y != oldY {
                    enemy.positionX = safe.x
                    enemy.positionY = safe.y
                    addLog("Spawn relocated for safety: (\(oldX), \(oldY)) -> (\(safe.x), \(safe.y))")
                }
                occupied.insert(tileKey(x: enemy.positionX, y: enemy.positionY))
            } else {
                addLog("Spawn safety error: no valid enemy tile for \(enemy.name) [\(index)].")
            }
        }

        for (index, pending) in pendingSpawns.enumerated() where pending.enemy.isAlive {
            let enemy = pending.enemy
            let oldX = enemy.positionX
            let oldY = enemy.positionY
            if let safe = findNearestValidSpawnTile(startX: oldX, startY: oldY, occupied: occupied, map: currentMissionTiles) {
                if safe.x != oldX || safe.y != oldY {
                ...
                addLog("Spawn safety error: no valid pending enemy tile for \(enemy.name) [\(index)].")
            }
        }
    }
    ...
    var missionObjectiveText: String {
        switch currentMissionType {
        case .stealth:
            return "Avoid detection and complete the run cleanly."
        case .assault:
            return "Push through resistance and secure the objective."
        case .extraction:
            return "Maintain momentum and reach extraction safely."
        }
    }

    var expectedThreatsText: String {
        switch currentMissionType {
        case .stealth:
            return "Watchers present. Detection risk high."
        case .assault:
            return "Enforcers present. Direct combat expected."
        case .extraction:
            return "Interceptors present. Movement pressure expected."
        }
    }

    var pressureProfileText: String {
        let attentionTotal = factionAttention[.corp, default: 0] + factionAttention[.gang, default: 0]
        switch attentionTotal {
        case 0...2:
            return "Low pressure expected."
        case 3...5:
            return "Moderate escalation likely."
        default:
            return "High escalation risk."
        }
    }

    var rewardProfileText: String {
        switch currentMissionType {
        case .stealth:
            return "Low trace yields bonus."
        case .assault:
            return "High intensity yields bonus."
        case .extraction:
            return "Balanced approach yields bonus."
        }
    }
    ...
    private func dominantArchetypeLabel(totalSpawnCount: Int) -> String? {
        guard totalSpawnCount > 0 else { return nil }
        var watcherCount = 0
        var enforcerCount = 0
        var interceptorCount = 0
        for index in 0..<totalSpawnCount {
    ...
    private func situationEnemyFlavorLine(totalSpawnCount: Int) -> String? {
        guard let dominant = dominantArchetypeLabel(totalSpawnCount: totalSpawnCount) else { return nil }
        switch (currentMissionType, dominant) {
        case (.stealth, "WATCHERS"), (_, "WATCHERS"):
            return "Watchers controlling narrow sightlines."
        case (.assault, "ENFORCERS"), (_, "ENFORCERS"):
            return "Enforcers dominating open space."
        case (.extraction, "INTERCEPTORS"), (_, "INTERCEPTORS"):
            return "Interceptors pressuring movement routes."
        default:
            return nil
        }
    }
    ...
    func setupMission(_ mission: Mission) {
        ...
        preparedMissionId = mission.id
        lastSetupTraceToken = "\(mission.id)::\(missionTypeLabel)::\(mapSituationLabel)"
        addLog("Setup trace: \(lastSetupTraceToken ?? "n/a")")
        ...
        addLog("Situation: \(mapSituationLabel) — \(situationShortHint)")
        addLog(missionStartSituationEffectLogLine())
        if let flavor = situationEnemyFlavorLine(totalSpawnCount: mission.enemies.count) {
            addLog(flavor)
        }
    ...
    func setupMultiRoomMission(_ mission: MultiRoomMission) {
        ...
        addLog("Situation: \(mapSituationLabel) — \(situationShortHint)")
        addLog(missionStartSituationEffectLogLine())
        if let flavor = situationEnemyFlavorLine(totalSpawnCount: firstRoom.enemies.count) {
            addLog(flavor)
        }
